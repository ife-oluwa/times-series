...
def extractData(date_list=getDates()) -> None:
...
def loadData() -> None:
    conn_string = "host=172.19.0.3 port=5432 dbname=sncf user=postgres password=docker"
...
dates = PythonOperator(task_id='Get_dates', python_callable=getDates, dag=dag)

extract = PythonOperator(
    task_id='Get_data', python_callable=extractData, dag=dag)

load = PythonOperator(task_id='Load_data', python_callable=loadData, dag=dag)
